/* Próximos passos:
        ...
        - Achar a linha central desse polígono
        - (X) Retornar para a base original (inverso do bird eyes)
        - Calcular o deslocamento em relação ao centro da câmera
*/
...
int main( int argc, char** argv )
{
    ...
    while ( (waitKey(33) != 27) && (cap.isOpened()) )
    {
        ...
        int x_histogram = histogram(color_img);

        sliding_img=bird_img.clone();
        sliding_Window(color_img, x_histogram, sliding_img);
