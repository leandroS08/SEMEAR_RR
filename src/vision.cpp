...
vector <Point2i> central_line_bird;
...
void histogram_Left(Mat& in)
{
    ...
    if ( (count_Den > 10) || (previous_left_x == -1))
    {
...
void navegation_Zone(Mat& in, Mat& out)
{
    ...
    for(int i= 0; i<navegavel.size(); i++){
        ...
        central_line_bird.push_back(Point(x_add,y_add));
    }
        
    polylines(out,navegavel_int,1,Scalar(255,0,0),2,8,0);
    polylines(out,central_line_bird,0,Scalar(0,255,255),2,8,0);
